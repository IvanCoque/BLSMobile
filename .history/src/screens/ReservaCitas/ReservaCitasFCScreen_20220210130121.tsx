...
export const ReservaCitasFCScreen: React.FC = () => {
  ...
  const onChangeStartTime = (event : any, selectedTime : any) => {
    ...
    const formatTime = new Date(selectedTime).getTimezoneOffset();
    console.log(new Date(selectedTime).getUTCHours())
    //JUEGO DE CEROS EN HORAS
    const horas = new Date(selectedTime).getUTCHours() <10 ? '0'+ new Date(selectedTime).getUTCHours();
    const fTime = new Date(selectedTime).getUTCHours() <10 ? '0'+new Date(selectedTime).getUTCHours() + ':' + new Date(selectedTime).getUTCMinutes() : new Date(selectedTime).getUTCHours() + ':' + new Date(selectedTime).getUTCMinutes();
    const currentTime = selectedTime || time;
    console.log(fTime);
    setShowStartTime(Platform.OS === 'ios');
    setStartTimeText(currentTime.toString());
