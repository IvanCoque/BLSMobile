...
export const ReservaCitasFCScreen: React.FC = () => {
  ...
  //ESPECIALIDADES
  const [especialidades, setEspecialidades] = useState<Consultorio[]>([]);
                    ...
                    >
                        <Picker.Item label="Seleccione un consultorio" value="0" />
                        {
                           Object.values(consultorios).map(data => (
                               <Picker.Item label={`${data.direccion_consultorio}`} value={[{id_consultorio : data.especialidades : JSON.stringify(data.id_especialidad)}]} />
