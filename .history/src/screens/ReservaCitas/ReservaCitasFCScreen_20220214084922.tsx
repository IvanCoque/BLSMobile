...
export const ReservaCitasFCScreen: React.FC = () => {
  ...
  const reservarCita = () =>{
    ...
    const fechaStartTime = `${fecha} ${startTimeText.substring(0,startTimeText.length-3)}.417-07:00`;
    const fechaEndTime = `${fecha} ${endTimeText.substring(0,endTimeText.length-3)}.417-07:00`;
   
    console.log(motivoReserva)
   
    console.log(new Date(fechaStartTime)) 
    const fechaStartTimeFormatted = format(new Date(fechaStartTime), 'yyyy-MM-dd HH:mm:ss');
    console.log(fechaStartTimeFormatted)
