...
export const ReservaCitasFCScreen: React.FC = () => {
  ...
  const onChangeStartTime = (event : any, selectedTime : any) => {
    ...
    const startTime = new Date(selectedTime).toUTCString()
    console.log(format(new Date(startTime, 'HH:mm:ss'))
    const currentTime = format(selectedTime, 'HH:mm:ss') || time;
    setShowStartTime(Platform.OS === 'ios');
    setStartTimeText(currentTime.toLocaleString());
