...
const ReservaCitasFCScreen: React.FC = () => {
  ...
  const renderItem = (item: ReservaCita) => {
      console.log('item desde render item')
        console.log(`${new Date(item.fecha_hora_fin_reserva).getHours()} - ${new Date(item.fecha_hora_fin_reserva).getMinutes()}-${new Date(item.fecha_hora_fin_reserva).getSeconds().toLocaleString()}`);
      return (
