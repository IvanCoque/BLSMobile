...
export const ReservaCitasFCScreen: React.FC = () => {
  ...
  const [horas, setHoras] = useState<{[key: string]: ReservaCita[]}>({});
  useEffect(() => {
    // run once
    ...
    //   );
        ...
        const mapHoras = () =>{
            ...
            data.map((reserva, index) => {
                console.log('desde mapeo de horas')
                console.log(reserva);
                setHoras({start : reserva.fecha_hora_inicio_reserva, end : reserva.fecha_hora_fin_reserva, title : reserva.motivo_reserva, summary : reserva.id_usuario.nombre})
            });
            setHoras(horas);
        }
        mapHoras();
